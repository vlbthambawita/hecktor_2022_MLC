...
from data.prepare_data_image import prepare_data, prepare_test_data
...
import segmentation_models_pytorch as smp
from my_models.triple_unet_plusplus import TripleUnetPlusPlus
...
parser.add_argument("--wandb_mode", type=str, help="wandb option", choices=["disabled", "online"], required=True)
...
parser.add_argument("--train_CSVs", 
                    default=["/work/vajira/DL/divergent-nets-hecktor/data/preparations/train_images_selected_blank_resampled.csv",
                    "/work/vajira/DL/divergent-nets-hecktor/data/preparations/train_images_with_multiclass_resampled.csv"
                    ],
                    help="CSV file list with image and mask paths")

parser.add_argument("--val_CSVs",
                    default=["/work/vajira/DL/divergent-nets-hecktor/data/preparations/test_images_with_only_multiclass_resampled.csv"],
                    help="CSV file list with image and mask paths")
...
parser.add_argument("--out_dir", 
                    default="/work/vajira/DATA/Hecktor_2022/checkpoint",
                    help="Main output dierectory")

parser.add_argument("--tensorboard_dir", 
                    default="/work/vajira/DATA/Hecktor_2022/tensorboard",
                    help="Folder to save output of tensorboard")

parser.add_argument("--test_out_dir",
                   default= "/work/vajira/DATA/Hecktor_2022/test_predictions",
...
parser.add_argument("--classes", default=[0,1,2], help="classes per pixel")
...
parser.add_argument("--grid_sizes_train", type=list, default=[512], help="Grid sizes to use in training")
parser.add_argument("--grid_sizes_val", type=list, default=[512], help="Grid sizes to use in training")
parser.add_argument("--grid_sizes_test", type=list, default=[512], help="Grid sizes to use in testing")
parser.add_argument("--in_channels", type=int, default=1, help="Number of input channgels")
parser.add_argument("--img_size", default=256, type=int, help="new value for resizing data in data augmentation")
...
run= wandb.init(project="hector", name= opt.py_file + "_" + opt.exp_name, mode=opt.wandb_mode)
run.config.update(opt) # write configurations
...
def train_model(train_loader, valid_loader, model, loss, metrics, optimizer, opt):
    ...
    for i in range(opt.start_epoch + 1, opt.start_epoch + opt.num_epochs +1 ):
        ...
        run.log({"train_logs":train_logs})
        run.log({"val_logs": valid_logs})
...
def run_train(opt):
    ...
    #loss = smp.utils.losses.JaccardLoss(ignore_channels=[0])
...
def do_test(opt):
    ...
    for i in range(opt.num_test_samples):
        ...
        fig = visualize(
            input_image_new_=np.transpose(image_vis, (1,2,0)).astype(int),
            GT_mask_0_=mask[0, :,:],
            Pred_mask_0_ = pr_mask[0,:,:],
            GT_mask_1_= mask[1,:,:],
            Pred_mask_1_ = pr_mask[1, :,:],
            GT_mask_2_= mask[2,:,:],
            Pred_mask_2_ = pr_mask[2, :,:]
        )

        fig.savefig(f"./test_202_{i}.png")
        #writer.add_figure(f"Test_sample/sample-{i}", fig, global_step=test_epoch)
        run.log({"sample_predictions": fig})
