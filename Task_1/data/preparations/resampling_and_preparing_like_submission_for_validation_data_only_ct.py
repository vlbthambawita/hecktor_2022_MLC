...
image_folder_original = Path("/work/vajira/DATA/Hecktor_2022/original_data/hecktor2022_training/hecktor2022/imagesTr").resolve()
image_folder_resampled = Path("/work/vajira/DATA/Hecktor_2022/resampled_data/training/imagesTr").resolve()
...
results_folder = Path("/work/vajira/DATA/Hecktor_2022/predictions_val_old_submission_new_preparation_method_v3").resolve() #predictions_val_old_submission
...
BEST_CHECK_POINT_PATH = "/work/vajira/DATA/Hecktor_2022/checkpoint/unet_plusplus_image_new_data_sampling.py/checkpoints/best_checkpoint.pth"
...
def predict(array_ct, array_pt):
    ...
    for slice_id in range(scaled_ct.shape[2]):
        ...
        slice_ct = np.expand_dims(slice_ct, axis=2)
        #print(slice_ct_pt_input.shape)
        #print(slice_ct.shape)
        res_slice = cv2.resize(slice_ct, dsize=(256, 256), interpolation=cv2.INTER_CUBIC) #use CT only
        #print(res_slice.shape)
        res_slice = np.expand_dims(res_slice, axis=2)
        ...
        #print("pred max cpu=",pred_max_cpu.shape)
        #pred_max_cpu_resized = cv2.resize(pred_max_cpu, dsize=(512, 512))
        #print("(res_slice.shape[0], res_slice.shape[1])=", (slice_ct.shape[0], slice_ct.shape[1]))
        pred_max_cpu = cv2.resize(pred_max_cpu, dsize=(slice_ct.shape[0], slice_ct.shape[1]), interpolation=cv2.INTER_CUBIC)
        pred_max_cpu_resized = np.rint(pred_max_cpu)
        pred_max_cpu_resized = np.clip(pred_max_cpu_resized, 0, 2)
        #print("pred max shape=", pred_max_cpu_resized.shape)
...
for i, row in tqdm(df.iterrows()):
    ...
    ct_resampled_space = image_ct.GetSpacing()
    print("ct origin space=", ct_origin_space)
    print("ct resampled  space=", ct_resampled_space)
    print("ct_origin_resampled = ", ct_origin_resampled)
    ...
    post_resampler.SetInterpolator(sitk.sitkNearestNeighbor) # new one
